...
func TestEvalIfElseExpressions(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if ok {
			testIntegerObject(t, eval, int64(expectedInt))
		} else {
...
func TestErrorHandling(t *testing.T) {
	tests := []struct {
		...
		{`{"name": "Monkey"}[fn(x) { x }];`, "type FUNC is not hashable"},
	}

	for _, tt := range tests {
		eval := testEval(tt.in)
		errorObj, ok := eval.(*object.Error)
		if !ok {
			t.Errorf("no error obj returned, got: %T, (%+v)", eval, eval)
			continue
		}
		if errorObj.Message != tt.expectedError {
			t.Errorf("wrong error message, expected: %s, got: %s", tt.expectedError, errorObj.Message)
		}
	}
}
...
func TestFunctionObject(t *testing.T) {
	...
	if len(fn.Parameters) != 1 {
		t.Fatalf("function has wrong no of parameters, expected 1 got: %d", len(fn.Parameters))
	}
...
func TestStringConcatenation(t *testing.T) {
	...
	str, ok := eval.(*object.String)
	if !ok {
		t.Fatalf("object is not of type *object.String, is: %T", eval)
	}
	if str.Value != "hello world" {
		t.Errorf("String object does not hold value %q, holds %q", input, str.Value)
	}
...
func TestHashLiterals(t *testing.T) {
	...
	if !ok {
		t.Fatalf("Eval didnt return hash, got=%T (%+v)", eval, eval)
	}
	...
	if len(result.Pairs) != len(expected) {
		t.Fatalf("Hash has wrong no of pairs, got=%d", len(result.Pairs))
	}
	for expKey, expVal := range expected {
		if pair, ok := result.Pairs[expKey]; ok {
